...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_status'])) {
    ...
    if (empty($name)) {
        $message = showError('لطفا نام وضعیت انتشار را وارد کنید.');
    } else {
        try {
            $stmt = $pdo->prepare("INSERT INTO content_publish_statuses (company_id, name, is_default, can_delete) VALUES (?, ?, ?, 1)");
            $stmt->execute([$selectedCompany, $name, $isDefault]);
            $message = showSuccess('وضعیت انتشار با موفقیت اضافه شد.');
        } catch (PDOException $e) {
            $message = showError('خطا در ثبت وضعیت انتشار: ' . $e->getMessage());
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_status'])) {
    ...
    if (empty($name)) {
        ...
        try {
            // بررسی اینکه آیا این وضعیت انتشار متعلق به شرکت کاربر است
            $canEdit = false;
            if (isAdmin()) {
                $canEdit = true;
            } else {
                $stmt = $pdo->prepare("SELECT company_id, can_delete FROM content_publish_statuses WHERE id = ?");
            ...
            if ($canEdit) {
                $stmt = $pdo->prepare("UPDATE content_publish_statuses SET name = ?, is_default = ? WHERE id = ?");
                $stmt->execute([$name, $isDefault, $statusId]);
                $message = showSuccess('وضعیت انتشار با موفقیت ویرایش شد.');
            } else {
                $message = showError('شما اجازه ویرایش این وضعیت انتشار را ندارید.');
            }
        } catch (PDOException $e) {
            $message = showError('خطا در ویرایش وضعیت انتشار: ' . $e->getMessage());
        }
    }
}
...
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    ...
    try {
        // بررسی اینکه آیا این وضعیت انتشار متعلق به شرکت کاربر است و قابل حذف است
        $canDelete = false;
        if (isAdmin()) {
            $stmt = $pdo->prepare("SELECT can_delete FROM content_publish_statuses WHERE id = ?");
            $stmt->execute([$statusId]);
            $status = $stmt->fetch();
        ...
        if ($canDelete) {
            // بررسی اینکه آیا این وضعیت انتشار در استفاده است
            $stmt = $pdo->prepare("SELECT COUNT(*) as count FROM contents WHERE publish_status_id = ?");
            $stmt->execute([$statusId]);
            $usageCount = $stmt->fetch()['count'];
            
            if ($usageCount > 0) {
                $message = showError('این وضعیت انتشار قابل حذف نیست زیرا در ' . $usageCount . ' محتوا استفاده شده است.');
            } else {
                $stmt = $pdo->prepare("DELETE FROM content_publish_statuses WHERE id = ?");
                $stmt->execute([$statusId]);
                $message = showSuccess('وضعیت انتشار با موفقیت حذف شد.');
            }
        } else {
            $message = showError('این وضعیت انتشار قابل حذف نیست یا شما اجازه حذف آن را ندارید.');
        }
    } catch (PDOException $e) {
        $message = showError('خطا در حذف وضعیت انتشار: ' . $e->getMessage());
    }
}
...
if (!empty($companyId)) {
    try {
        ...
        $stmt = $pdo->prepare("SELECT COUNT(*) as count FROM content_publish_statuses WHERE company_id = ? AND name IN ('منتشر شده', 'منتشر نشده')");
        $stmt->execute([$companyId]);
        $defaultStatusesCount = $stmt->fetch()['count'];
        
        // اگر وضعیت‌های پیش‌فرض وجود ندارند، آنها را اضافه کن
        if ($defaultStatusesCount < 2) {
            $defaultStatuses = [
                ['منتشر شده', 0, 0], // name, is_default, can_delete
                ['منتشر نشده', 1, 0]
            ];
            
            $insertStmt = $pdo->prepare("INSERT INTO content_publish_statuses (company_id, name, is_default, can_delete) VALUES (?, ?, ?, ?)");
            
            foreach ($defaultStatuses as $status) {
                // بررسی وجود وضعیت
                $checkStmt = $pdo->prepare("SELECT id FROM content_publish_statuses WHERE company_id = ? AND name = ?");
...
// دریافت لیست وضعیت‌های انتشار
$query = "SELECT s.*, c.name as company_name,
          (SELECT COUNT(*) FROM contents WHERE publish_status_id = s.id) as usage_count 
          FROM content_publish_statuses s 
...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <div>
        <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addStatusModal">
            <i class="fas fa-plus"></i> افزودن وضعیت انتشار جدید
        </button>
...
<div class="card">
    <div class="card-body">
                                ...
                                <td>
                                    ...
                                    <?php if ($status['can_delete'] && $status['usage_count'] == 0): ?>
                                        <a href="?<?php echo isAdmin() && !empty($companyId) ? 'company=' . $companyId . '&' : ''; ?>delete=<?php echo $status['id']; ?>" 
                                           ...
                                           onclick="return confirm('آیا از حذف این وضعیت انتشار اطمینان دارید؟')">
        ...
        <?php else: ?>
            <p class="text-center">هیچ وضعیت انتشاری یافت نشد.</p>
        <?php endif; ?>
    </div>
</div>
        ...
        <div class="modal-content">
            ...
            <form method="POST" action="">
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label for="name" class="form-label">نام وضعیت انتشار</label>
        ...
        <div class="modal-content">
            ...
            <form method="POST" action="">
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label for="edit_name" class="form-label">نام وضعیت انتشار</label>
                    ...
                    <div class="mb-3" id="can_delete_info">
                        <div class="alert alert-warning">
                            ...
                            این وضعیت انتشار پیش‌فرض سیستم است و قابل حذف نیست.
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
